...
cache = Cache()
admin = Admin(name='RippleApp', template_mode='bootstrap3')
...
def create_app():
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:////opt/ripple/instance/entries.db'
    ...
    admin.init_app(app)
    app.config['CACHE_TYPE'] = 'simple'  # Use 'simple' cache type for simplicity
    cache.init_app(app)
    ...
    admin.add_view(ModelView(Emotions, db.session))
...
def register():
    ...
    if request.method == "POST":
        ...
        app.logger.info(f"Received data: {username}, {password}, {confirmation}")
